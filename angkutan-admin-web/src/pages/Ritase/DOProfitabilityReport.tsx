...
const DOProfitabilityReport: React.FC = () => {
  ...
  const fetchAddresses = useCallback(async (data: ProfitabilityData[]) => {
    ...
    const addressPromises = data.map(async (item) => {
      const loadAddress =
        item.load_latitude && item.load_longitude
      ...
      const unloadAddress =
        item.unload_latitude && item.unload_longitude
          ? await getAddressFromCoordinates(
              item.unload_latitude,
              item.unload_longitude
            )
          : item.unload_location || "Unload address not set";

      return {
        id: item.id,
        addresses: { load: loadAddress, unload: unloadAddress },
      };
    });
  ...
  const filteredData = useMemo(() => {
    return reportData.filter((item) => {
      ...
      const searchTermMatch =
        ...
        (String(addresses[item.id]) && (addresses[String(item.id)]?.load || ""))
          .toLowerCase()
          .includes(searchTerm.toLowerCase()) ||
        (
          String(addresses[item.id]) &&
          (addresses[String(item.id)]?.unload || "")
        )
          .toLowerCase()
          .includes(searchTerm.toLowerCase());

      // Load / Unload location filters (both must match when provided)
      const loadField = (
        addresses[String(item.id)]?.load ||
        item.load_location ||
        ""
      ).toLowerCase();
      const unloadField = (
      ...
      const loadFilterMatch =
        loadLocationFilter === "" ||
        loadField.includes(loadLocationFilter.toLowerCase());
      const unloadFilterMatch =
        unloadLocationFilter === "" ||
        unloadField.includes(unloadLocationFilter.toLowerCase());

      return (
        poFilterMatch && searchTermMatch && loadFilterMatch && unloadFilterMatch
      );
    });
  }, [
    reportData,
              ...
              <tr key={item.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-normal text-sm text-gray-600">
                  <div className="font-semibold">
                    ...
                    {isAddressLoading
                      ? "Loading..."
                      : addresses[item.id]?.load || "N/A"}
                  </div>
                  <div className="mt-1">
                    <span className="text-xs text-gray-500">Bongkar: </span>
                    {isAddressLoading
                      ? "Loading..."
                      : addresses[item.id]?.unload || "N/A"}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                ...
                <td
                  className={`px-6 py-4 whitespace-nowrap text-sm font-bold text-right ${
                    item.netProfit >= 0 ? "text-blue-600" : "text-orange-600"
                  }`}
                >
      ...
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className="text-gray-600">
            Financial overview of each delivery order.
          </p>
        </div>
      </div>
      ...
      <div className="mb-4 p-4 bg-white rounded-lg shadow-sm border border-gray-200 flex items-center gap-4">
        <input
          ...
          placeholder={
            isAddressLoading
              ? "Loading addresses..."
              : "Search by location, customer, driver..."
          }
          value={searchTerm}
        ...
        <input
          type="text"
          placeholder="Filter Load Location"
          value={loadLocationFilter}
          onChange={(e) => setLoadLocationFilter(e.target.value)}
          disabled={isAddressLoading}
          className="w-64 p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-200"
        />
        <input
          type="text"
          placeholder="Filter Unload Location"
          value={unloadLocationFilter}
          onChange={(e) => setUnloadLocationFilter(e.target.value)}
          disabled={isAddressLoading}
          className="w-64 p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-200"
        />
        <select
