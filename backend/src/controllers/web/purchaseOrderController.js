...
exports.getAllPurchaseOrders = async (req, res, next) => {
  try {
        ...
        const depositGroup = po.depositGroup
          ? {
              ...
              remaining_quantity: parseFloat(
                po.depositGroup.remaining_quantity || 0
              ),
    ...
    const depositStats = {
      linked_to_deposit: enhancedPOs.filter((po) => po.is_deposit_linked)
        .length,
...
exports.updatePurchaseOrder = async (req, res, next) => {
  ...
  try {
    ...
    if (
      deposit_group_id !== undefined &&
      deposit_group_id !== po.deposit_group_id
    ) {
      if (deposit_group_id) {
        const group = await DepositGroup.findByPk(deposit_group_id, {
          transaction,
        });
        if (!group) {
          await transaction.rollback();
    ...
    if (updateData.total_quantity) {
      ...
      if (po.deposit_group_id) {
        ...
        if (group) {
          ...
          const currentRemaining = parseFloat(group.remaining_quantity || 0);
          // Update target to match PO total; adjust remaining by the delta to preserve progress
          group.target_quantity = newQuantity;
          group.remaining_quantity = currentRemaining + delta;
    ...
    if (updateData.deposit_group_id) {
      ...
      for (const doItem of existingDOs) {
        ...
        if (!isAlreadyInGroup) {
          ...
          console.log(
            `✅ Retroactively added DO ${doItem.do_number} to deposit group ${updateData.deposit_group_id}`
          );
        }
      }
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        has_location_data: po.hasLocationData
          ? po.hasLocationData()
...
exports.createDeliveryOrderFromPO = async (req, res, next) => {
  ...
  try {
    ...
    if (po.deposit_group_id) {
      ...
      console.log(
        `✅ Auto-added new DO ${newDO.do_number} to deposit group ${po.deposit_group_id}`
      );
    }
    // *** FIX ENDS HERE ***
...
exports.getRecentCustomers = async (req, res, next) => {
  try {
    const recentPOs = await PurchaseOrder.findAll({
      ...
      order: [["created_at", "DESC"]],
      limit: 50, // Ambil 50 PO terakhir untuk memastikan kita dapat 5 nama unik
      raw: true,
    });

    // Buat daftar nama unik dan ambil 5 teratas
    const uniqueCustomerNames = [
      ...new Set(recentPOs.map((po) => po.customer_name)),
    ];
