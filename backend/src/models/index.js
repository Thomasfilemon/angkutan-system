...
// Load all your model setup functions
...
const setupAdminProfileModel = require('./adminProfile.model'); // <-- Added this crucial import
...
const sequelize = new Sequelize(process.env.DB_NAME, process.env.DB_USER, process.env.DB_PASSWORD, {
  ...
  pool: { // Added a connection pool for better performance and stability
    max: 5,
    min: 0,
    acquire: 30000,
    idle: 10000
...
db.AdminProfile = setupAdminProfileModel(sequelize); // <-- Added AdminProfile model
...
PurchaseOrder.hasMany(DeliveryOrder, { foreignKey: 'purchase_order_id', as: 'deliveryOrders' });
DeliveryOrder.belongsTo(PurchaseOrder, { foreignKey: 'purchase_order_id', as: 'purchaseOrder' });
...
Vehicle.hasMany(DeliveryOrder, { foreignKey: 'vehicle_id', as: 'deliveryOrders' });
