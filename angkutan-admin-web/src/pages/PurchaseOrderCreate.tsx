...
const PurchaseOrderCreatePage = () => {
  ...
  const [showPreview, setShowPreview] = useState(true); // Toggle for preview
  ...
  const [poNumberPreview, setPoNumberPreview] = useState(""); // Frontend suggestion

  // Suggested items (stub—fetch from API if you have one)
  ...
  // Auto-gen PO number preview
  useEffect(() => {
    const today = new Date();
    const yearMonth = `${today.getFullYear()}${String(
      today.getMonth() + 1
    ).padStart(2, "0")}`;
    setPoNumberPreview(`PO-${yearMonth}-XXX`); // XXX for seq—backend sets real
  }, []);

  // Smart item handling: Add on comma/Enter, suggest on type, prevent dups
  ...
  const handleCreatePO = async () => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || "Failed to create PO";
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="relative group">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  ...
                  <span
                    className="ml-1 text-gray-400 cursor-help"
                    title="Enter full customer name."
                  >
                    ℹ️
                  </span>
                </label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div className="relative group">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    <span
                      className="ml-1 text-gray-400 cursor-help"
                      title="Choose based on measurement type."
                    >
                      ℹ️
                    </span>
                  </label>
