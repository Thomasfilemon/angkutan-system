      ...
      {selectedGroup ? (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <div className="font-medium">
                  {selectedGroup.target_quantity?.toLocaleString("id-ID") || 0}{" "}
              ...
              <div>
                ...
                <div className="font-medium">
                  {selectedGroup.remaining_quantity?.toLocaleString("id-ID") ||
                    0}{" "}
                  {getUnitDisplay(selectedGroup.unit || "ton")}
                </div>
              </div>
                          ...
                          {po.do_count === 0 && (
                            <button
                              ...
                              onClick={() => {
                                ...
                                window.open(
                                  `/admin/purchase-orders/${po.id}/create-do`,
                                  "_blank"
                                );
                              }}
                            >
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
              ...
              <div className="space-x-2">
                {(!selectedGroup.linkedPOs ||
                  selectedGroup.linkedPOs.length === 0) && (
                  <button
                    onClick={() => setShowLinkPOModal(true)}
                    className="bg-indigo-500 hover:bg-indigo-700 text-white px-4 py-2 rounded text-sm"
                  >
                      ...
                      <tr key={member.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          {member.quantity > doItem.minimal_load_quantity ? (
                            <button
                              onClick={() =>
                                generateSelisihInvoiceForDO(member)
                              }
                              disabled={creatingInvoiceDoId === doItem.id}
                              className={`px-3 py-1 rounded text-white ${
                                printedSelisihDoIds.has(doItem.id)
                                  ? "bg-green-600"
                                  : "bg-indigo-600 hover:bg-indigo-700"
                              } disabled:opacity-50`}
                            >
                              {creatingInvoiceDoId === doItem.id
                                ? "Membuat..."
                                : printedSelisihDoIds.has(doItem.id)
                                ? "Invoice Dicetak"
                                : "Cetak Invoice Selisih"}
                            </button>
                          ) : (
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
              <h3 className="text-lg font-medium">
                Tagihan Selisih (Extra Quantity)
              </h3>
              <button
            ...
            selectedGroup.selisih_status === "paid" ? (
              <div className="p-6">
                <div
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <p className="font-semibold text-gray-800">
                        Total Tagihan:{" "}
                        {formatCurrency(
                          selectedGroup.total_selisih_amount || 0
                        )}
                      </p>
                  ...
                  <div className="overflow-x-auto">
                    {(() => {
                      const rows = parseSelisihDetails(
                        selectedGroup.selisih_details
                      );
                      const findMember = (doNumber: string) =>
                        selectedGroup.members.find(
                          (m) => m.deliveryOrder?.do_number === doNumber
                        );
              ...
              <div className="text-center py-8">
                <p className="text-gray-500">
                  Tidak ada tagihan selisih untuk group ini.
                </p>
              </div>
            )}
          </div>
