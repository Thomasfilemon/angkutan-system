-- 1. Pengguna & Profil
-- password untuk semua adalah 'password123'
...
-- 4. Membuat beberapa Delivery Orders di bawah PO yang sama
-- Mengambil data dari contoh Excel
INSERT INTO delivery_orders 
(purchase_order_id, driver_id, vehicle_id, do_number, customer_name, item_name, quantity, unit_price, total_amount, payment_status, due_date, load_location, unload_location) 
VALUES
-- DO 1 (Untuk Supir Andi)
((SELECT id FROM purchase_orders WHERE po_number = 'PO/WIKA/09/2024-01'), 
 ...
 'DO-240928-01', 'PT WIKA BETON', 'Abu Batu', 34.05, 155000, 5277750, 'proses_tagihan', '2024-10-28', 'Quarry Jonggol', 'Proyek Tol Cibitung'),
-- DO 2 (Untuk Supir Budi)
((SELECT id FROM purchase_orders WHERE po_number = 'PO/WIKA/09/2024-01'), 
 (SELECT id FROM users WHERE username = 'supir_budi'), 
 (SELECT id FROM vehicles WHERE license_plate = 'B 5678 DEF'), 
 'DO-240928-02', 'PT WIKA BETON', 'Abu Batu', 34.72, 155000, 5381600, 'proses_tagihan', '2024-10-28', 'Quarry Jonggol', 'Proyek Tol Cibitung'),
-- DO 3 (Untuk Supir Charlie, status lunas dengan deposit)
((SELECT id FROM purchase_orders WHERE po_number = 'PO/WIKA/09/2024-01'), 
 (SELECT id FROM users WHERE username = 'supir_charlie'), 
 (SELECT id FROM vehicles WHERE license_plate = 'B 9012 GHI'), 
 'DO-240928-03', 'PT WIKA BETON', 'Split 1-2', 33.53, 155000, 5197150, 'deposit', '2024-10-28', 'Quarry Jonggol', 'Proyek Tol Cibitung');

-- 5. Contoh Biaya Supir untuk salah satu DO
-- Supir Andi melaporkan biaya untuk DO-240928-01
INSERT INTO driver_expenses (delivery_order_id, driver_id, jenis, amount) VALUES
((SELECT id FROM delivery_orders WHERE do_number = 'DO-240928-01'),
 (SELECT id FROM users WHERE username = 'supir_andi'),
 'Uang Jalan', 1700000),
((SELECT id FROM delivery_orders WHERE do_number = 'DO-240928-01'),
 (SELECT id FROM users WHERE username = 'supir_andi'),
...
-- 6. Contoh Pencatatan Akuntansi Ritase untuk DO yang sama
