...
const IconAdjust = () => (
  ...
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M12 6V4m0 16v-2m8-8h2M4 12H2m15.364 6.364l-1.414-1.414M6.05 6.05l-1.414-1.414m12.728 0l-1.414 1.414M6.05 17.95l-1.414 1.414"
    />
  </svg>
);
const IconRestock = () => (
  ...
  >
    ...
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M4 9a9 9 0 0114.65-4.65L20 5M20 15a9 9 0 01-14.65 4.65L4 19"
    />
  </svg>
);
const IconEdit = () => (
...
interface StockItem {
  ...
  batch_count?: number;
  is_low_stock?: boolean; // ✅ New field for low stock status
}
...
const StockManagementPage = () => {
  ...
  const [selectedItemForBatches, setSelectedItemForBatches] =
  ...
  const debouncedFetch = useCallback(
    debounce((...args) => fetchStockItems(...args), 500),
    [fetchStockItems]
  );
    ...
    try {
      if (selectedItemForRestock) {
        await apiClient.post(
          `/stock/${selectedItemForRestock.id}/add`,
          restockData
        );
                  ...
                  <td className="px-6 py-4 border border-gray-300">
                    ...
                    <div className="flex justify-center items-center space-x-1">
                      ...
                      <Link
                        ...
                        onClick={() =>
                          console.log(
                            "🚀 Navigating to batches for item:",
                            item.id
                          )
                        }
        ...
        <span className="text-sm text-gray-700">
          ...
          {stockItems.filter((item) => item.is_low_stock).length > 0 && (
            <span className="ml-4 text-yellow-600">
              • {stockItems.filter((item) => item.is_low_stock).length} stok
              rendah
            </span>
          )}
        </span>
              ...
              <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded text-sm">
                ...
                <p>
                  ...
                  {Array.isArray(batches)
                    ? `Array with ${batches.length} items`
                    : "Not array"}
                </p>
                <p>
                  First batch:{" "}
                  {batches[0]
                    ? JSON.stringify(batches[0].batch_number)
                    : "None"}
                </p>
              </div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    ...
                    {batchLoading ? (
                      <tr>
                        <td
                          colSpan={7}
                          className="text-center p-8 text-blue-600"
                        >
                          ⏳ Memuat data batch...
                        </td>
                      </tr>
                      ...
                      batches.map((batch) => (
                        <tr key={batch.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 text-sm text-gray-900 font-semibold text-green-600">
                            Rp{" "}
                            {Number(batch.purchase_price).toLocaleString(
                              "id-ID"
                            )}
                          </td>
                          <td className="px-6 py-4 text-sm text-gray-900">
                            {batch.initial_quantity}{" "}
                            {selectedItemForBatches.unit}
                          </td>
                          <td className="px-6 py-4 text-sm text-gray-900">
                            <span
                              className={`font-semibold ${
